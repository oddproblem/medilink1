...
class SymptomPredictionScreen extends StatefulWidget {
  ...
  @override
  State<SymptomPredictionScreen> createState() => _SymptomPredictionScreenState();
}
...
class _SymptomPredictionScreenState extends State<SymptomPredictionScreen> {
  ...
  Future<void> _predictDisease() async {
    ...
    try {
      ...
      final postUrl = Uri.parse('https://raushan2709-disease-prediction-workers.hf.space/gradio_api/call/predict_disease');
      final postPayload = {
        "data": [
      ...
      if (postResponse.statusCode != 200) {
        throw Exception('Failed to initiate prediction (Status: ${postResponse.statusCode})');
      }
      ...
      final getUrl = Uri.parse('https://raushan2709-disease-prediction-workers.hf.space/gradio_api/call/predict_disease/$eventId');
      final getResponse = await http.get(getUrl).timeout(const Duration(seconds: 40));
      ...
      if (getResponse.statusCode == 200) {
        ...
        if (prediction != null) {
          ...
          throw Exception('Failed to parse prediction result from server output');
        }
      } else {
        throw Exception('Failed to retrieve prediction result (Status: ${getResponse.statusCode})');
      }
    } catch (e) {
      setState(() {
        _error = 'Prediction failed: $e. Make sure the Hugging Face space is active.';
        _isLoading = false;
      });
    }
  }
  ...
  Widget build(BuildContext context) {
    ...
    final filteredSymptoms = _allSymptoms.where((symptom) {
      final readable = symptom.replaceAll('_', ' ').toLowerCase();
      return readable.contains(_searchText.toLowerCase());
    }).toList();

    return Scaffold(
      backgroundColor: const Color(0xFF0F172A), // Slate 900
      appBar: AppBar(
        title: const Text('AI Symptom Predictor'),
        ...
        actions: [
          if (_selectedSymptoms.isNotEmpty)
            TextButton(
              ...
              child: const Text('Clear All', style: TextStyle(color: Colors.cyan)),
            ),
        ],
      ),
      body: Padding(
        ...
        child: Column(
          children: [
            const Text(
              'Select all symptoms you are experiencing to predict potential illnesses.',
            ...
            if (_isLoading) ...[
              ...
              Container(
                ...
                child: Column(
                  children: [
                    const Text(
                      'PREDICTED ILLNESS / CONDITION:',
                      style: TextStyle(color: Colors.cyan, fontSize: 11, fontWeight: FontWeight.bold, letterSpacing: 1),
                    ),
                    const SizedBox(height: 8),
                    Text(
                      _predictedDisease!,
                    ...
                    const SizedBox(height: 12),
                    const Text(
                      'Disclaimer: This prediction is based on statistical correlation models and should not replace clinical diagnosis. Please consult a doctor.',
                ...
                children: [
                  Expanded(
                    child: ElevatedButton.icon(
                      onPressed: _selectedSymptoms.isEmpty ? null : _predictDisease,
